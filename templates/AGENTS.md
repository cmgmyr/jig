...
## Ownership
- Chris owns each README's Premise and Decisions. Propose edits; apply only after he approves.
...
3. Run `wb-stale`. Mention at most two stale entries and ask: promote, park, or keep? Don't push if he says skip.
...
- New idea mentioned at the root: run `wb-new`, then ask Chris for the premise in one sentence. Don't invent it.
