---
name: workbench-file
description: File the current conversation or session into a workbench entry. Use when the human says "file this", "save this to the workbench", "log this session", "put this in the workbench", or wants decisions and sources from this chat recorded durably.
---
...
3. Distill the session into three buckets:
   ...
   - **README edits.** Proposed changes to `## Open questions` and `## Next experiment` only. Do not propose Premise or Decisions edits unless the human asked for them.
...
5. Show the proposed README edits as a diff and wait for approval. Apply only what the human approves.
6. Print what was written. Do not commit; the workbench commits on its own.

## Rules

- Never change `status:` by hand. Use `wb-set <slug> <status> ["outcome"]` and only when the human asks.
- Never edit `INDEX.md`.
